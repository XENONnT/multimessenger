...
class Plotter:
    ...
    def plot_3cols(self, _y, _x,
        ...
        if ydim == 2:  # i.e. data has multiple bins
            return self._plot_raw_data(fig, axes, _y, _x, cmap, ave_over, average)
        elif ydim == 1:  # i.e. data is integrated over one axis
            return self._plot_integrated(fig, axes, _y, _x)
    ...
    def _plot_raw_data(self, fig, axes, _y, _x, cmap, ave_over, average):
        ...
        if average:
            ...
            if type(axes) == np.ndarray:
                zipped = zip(axes, _y.items())
            elif type(axes) != np.ndarray:
                zipped = zip([axes] * 3, _y.items())
            else:
                raise ValueError("Unsupported axes structure")
    ...
    def _plot_integrated(self, fig, axes, _y, _x):
    ...
    def plot_data(self, x='time', y='Luminosity', ftype='horizontal',
        ...
        **kwargs
        """
        if x.lower() == 'time':
            _x, xlabel = self.Model.t, 'time [s]'
        elif x.lower() == 'energy':
            _x, xlabel = self.Model.mean_E, r'$\langle E_\nu \rangle$ [MeV]'
        else:
            raise ValueError(f'{x} not recognized')

        if y.lower() == 'luminosity':
            _y, ylabel = self.Model.L_list, 'Luminosity [erg/s/MeV]'
        elif y.lower() == 'number':
            _y, ylabel = self.Model.nu_list, r'$\nu$ number [cts/s/MeV]'
        else:
            return print(f'{y} not recognized')
        ...
        axs = adjust_xylabels(ftype, axs, xlabel, ylabel, xscale, yscale)
        return fig, axs
    ...
    def plot_sampled_energies(self,
        ...
        if x.lower() == 'energy':
            spectrum, xaxis = self.Model.total_rate1D, self.Model.recoil_en
        elif x.lower() == 'time':
        ...
        ax = adjust_xylabels('single', ax, xl, 'counts', xscale, yscale)
