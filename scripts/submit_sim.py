...
parser.add_argument('-N', '--ntotal', help='Number of realizations to simulate. For N=M, simulates the model M times.',
                    type=int, defaul=1, required=False)
...
def make_batch_script(model_name,model_index,distance,volume,runid, incr):
    main_str = f'''#!/bin/bash
#SBATCH --qos=xenon1t
...
python simulate_snmodel.py -m {model_name} -i {model_index} -d {distance} -v {volume} -id {runid}
EOF''' 
...
if __name__ == "__main__":
    ...
    for i in range(ntotal):
        incr = f"{i:03}"
        make_batch_script(model_name, model_index, distance, volume, runidbase, incr)
