...
customstyle = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "customstyle.mplstyle"
)
...
class InteractionSingle:
    ...
    def __init__(self, Model, Target, recoil_energies):
        ...
        self.xsecs = self.Target.nN_cross_section(
            self.neutrino_energies, self.recoil_energies
        )
    ...
    def dRdEr(self):
        ...
        time_integrated_fluxes = {
            f: np.trapz(self.Model.fluxes[f], self.times, axis=0).to(1 / u.keV)
            for f in Flavor
        }
        ...
        rates_per_recoil_single = {
            f: self.Target.abund
            * np.trapz(integrand[f], self.neutrino_energies, axis=1)
            for f in Flavor
        }
        # change the units and return
        rates_per_recoil_single = {
            k: v.to(u.m**2 / u.keV) for k, v in rates_per_recoil_single.items()
        }
    ...
    def dRdt(self):
        ...
        # change the units and return
        rates_per_time_single = {
            k: v.to(u.m**2 / u.s) for k, v in rates_per_time_single.items()
        }
...
class Interactions:
    def __init__(self, Model, Nuclei="Xenon", isotope="mix", _ignore_if_exists=False):
        ...
        self.all_targets = [
            InteractionSingle(self.Model, t, self.recoil_energies) for t in self.Nucleus
        ]
    ...
    def save_object(self, update=False):
        ...
        if update:
            ...
            with open(full_file_path, "wb") as output:  # Overwrites any existing file.
                ...
                click.secho(
                    f"> Saved at <self.proc_loc>/{self.interaction_name}!\n", fg="green"
                )
    ...
    def compute_interaction_rates(
        self, recoil_energies=None, force=False, leave_bar=False, **kw_model
    ):
        """
        ...
        **kw_model keyword arguments for the model in order to compute the fluxes
        ...
        if self.Model.fluxes is None:
            click.secho(
                f"\t> Fluxes is not computed for {self.Model.object_name}, \n\tcomputing now...",
                fg="blue",
            )
        ...
        if self.rates_per_recoil is not None and not force:
            click.secho(
                f"\t> Recoil rates have been found! Stored in self.rates_per_recoil & self.rates_per_time"
            )
            # rates have been computed
            return None

        # interaction rates for each isotope
        self.rates_per_recoil_iso = {
            isotope.name: isotope.dRdEr()
            for isotope in tqdm(self.all_targets, leave=leave_bar)
        }
        self.rates_per_time_iso = {
            isotope.name: isotope.dRdt()
            for isotope in tqdm(self.all_targets, leave=leave_bar)
        }
    ...
    def _compute_total_rates(self):
        ...
        # add the total
        self.rates_per_recoil["Total"] = np.zeros_like(
            self.rates_per_recoil[Flavor.NU_E]
        )
        ...
        click.secho(
            f"> Computed the total rates at the source for 1 atom (not scaled)",
            fg="blue",
        )
    ...
    def scale_rates(self, distance, volume, N_Xe=4.6e27 * u.count / u.tonne):
        ...
        try:
            ...
            raise NotImplementedError(
                "\t> Rates haven't been computed yet!\n"
    ...
    def plot_rates(self, scaled=True):
        ...
        if scaled:
            if type(self.rates_per_time_scaled) == type(None):
                click.secho(
                    f"\t>Scaled rates have not been computed!, Use self.scale_rates()",
                    fg="red",
                )
                return None
            ...
            if type(self.rates_per_recoil) == type(None):
                click.secho(
                    f"\t>Rates have not been computed!, Use self.compute_interaction_rates()",
                    fg="red",
                )
                return None
